...
@section('content')
    <section class="bg-[#1A1A23] rounded-3xl border border-white/5 overflow-hidden animate-fade-in-up">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 px-6 py-6 border-b border-white/5 bg-[#12121A]/50 backdrop-blur-xl">
            <div>
                ...
                <h3 class="text-2xl font-bold text-white flex items-center gap-2">
                    <i class="fa-solid fa-baby text-purple-500"></i> Data Kelahiran
                </h3>
                <p class="text-sm text-gray-400 mt-1">Catatan kelahiran warga dan bukti dokumen.</p>
            </div>
            ...
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="flex items-center w-full sm:w-64 bg-[#0B0B0F] rounded-xl px-4 py-2.5 border border-white/10 focus-within:border-purple-500/50 transition-colors">
                    ...
                    <input type="text" name="q" value="{{ $search }}" class="flex-1 bg-transparent text-sm text-white placeholder-gray-600 focus:outline-none" placeholder="Cari nama/no akta...">
                </div>
                <input type="text" name="no_akta" value="{{ $noAkta }}" class="w-full sm:w-40 bg-[#0B0B0F] rounded-xl px-4 py-2.5 border border-white/10 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/50 transition-colors" placeholder="No Akta">
                <input type="text" name="tempat" value="{{ $tempat }}" class="w-full sm:w-40 bg-[#0B0B0F] rounded-xl px-4 py-2.5 border border-white/10 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/50 transition-colors" placeholder="Tempat Lahir">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse ($kelahiran as $item)
                    ...
                    <div class="group bg-[#0B0B0F] rounded-3xl border border-white/5 overflow-hidden hover:border-purple-500/30 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-purple-500/10">
                        <div class="p-6">
                            <div class="flex items-start gap-4 mb-6">
                                <div class="w-16 h-16 rounded-2xl bg-[#1A1A23] border border-white/10 flex items-center justify-center overflow-hidden flex-shrink-0 relative group-hover:border-purple-500/50 transition-colors">
                                    @if($fileUrl)
                                        ...
                                        @endif
                                    @else
                                        <i class="fa-solid fa-baby text-2xl text-gray-600"></i>
                                    @endif
                                </div>
                                <div>
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between text-sm border-b border-white/5 pb-2">
                                    ...
                                    <span class="text-gray-300 font-medium">{{ $item->tgl_lahir }}</span>
                                </div>
                                <div class="flex items-center justify-between text-sm border-b border-white/5 pb-2">
                                    <span class="text-gray-500">Tempat Lahir</span>
                                    <span class="text-gray-300">{{ $item->tempat_lahir }}</span>
                                </div>
                                <div class="flex items-center justify-between text-sm pt-1">
                                    <span class="text-gray-500">No Akta</span>
                                    <span class="text-gray-300 font-mono">{{ $item->no_akta }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="px-6 py-4 bg-[#12121A] border-t border-white/5 flex justify-between items-center">
                            ...
                            <a href="{{ route('guest.kelahiran.show', $item->kelahiran_id) }}" class="text-sm font-medium text-purple-400 hover:text-purple-300 flex items-center gap-1 transition-colors">
                ...
                @empty
                    <div class="col-span-full py-12 text-center">
                        ...
                        <p class="text-gray-500">Data kelahiran belum tersedia saat ini.</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="px-6 py-4 border-t border-white/5 bg-[#12121A]">
            {{ $kelahiran->links('pagination::tailwind') }}
        </div>
    </section>
@endsection
