    ...
    <section class="bg-[#1A1A23] rounded-3xl border border-white/5 overflow-hidden animate-fade-in-up">
        <div class="px-6 py-6 border-b border-white/5 bg-[#12121A]/50 backdrop-blur-xl flex items-center justify-between">
            <div>
                ...
                <h2 class="text-2xl font-bold text-white">Data Kematian #{{ $kematian->kematian_id }}</h2>
            </div>
        ...
        <div class="p-6 md:p-8 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div class="bg-[#0B0B0F] p-5 rounded-2xl border border-white/5 hover:border-purple-500/20 transition-colors group lg:col-span-3">
                    ...
                    <p class="text-white text-lg font-bold group-hover:text-purple-400 transition-colors">{{ $kematian->warga->nama ?? '-' }}</p>
                </div>
                <div class="bg-[#0B0B0F] p-5 rounded-2xl border border-white/5 hover:border-purple-500/20 transition-colors">
                    <p class="text-xs text-gray-500 uppercase font-bold tracking-wider mb-2">Tanggal Meninggal</p>
                    <p class="text-white font-mono flex items-center gap-2">
                        <i class="fa-regular fa-calendar text-purple-400"></i>
                        {{ $kematian->tgl_meninggal }}
                    </p>
                </div>
                <div class="bg-[#0B0B0F] p-5 rounded-2xl border border-white/5 hover:border-purple-500/20 transition-colors">
                    <p class="text-xs text-gray-500 uppercase font-bold tracking-wider mb-2">Penyebab</p>
                    <p class="text-white">{{ $kematian->sebab }}</p>
                </div>
                <div class="bg-[#0B0B0F] p-5 rounded-2xl border border-white/5 hover:border-purple-500/20 transition-colors">
                    <p class="text-xs text-gray-500 uppercase font-bold tracking-wider mb-2">Lokasi</p>
                    <p class="text-white">{{ $kematian->lokasi }}</p>
                </div>
                <div class="bg-[#0B0B0F] p-5 rounded-2xl border border-white/5 hover:border-purple-500/20 transition-colors lg:col-span-3">
                    <p class="text-xs text-gray-500 uppercase font-bold tracking-wider mb-2">No. Surat Kematian</p>
                    <p class="text-white font-mono tracking-wide">{{ $kematian->no_surat }}</p>
                </div>
            </div>
            ...
            <div class="mt-8 flex gap-3 pt-6 border-t border-white/5">
                <a href="{{ route('guest.kematian.index') }}" class="px-5 py-2.5 rounded-xl bg-[#0B0B0F] border border-white/10 text-gray-300 hover:text-white hover:bg-[#1A1A23] transition-all font-medium text-sm flex items-center gap-2">
